...
export function UpgradeSection() {
  return (
    <section className="rounded-2xl glass-card-strong p-6 sm:p-10 relative overflow-hidden glow-ring animate-fade-in" style={{ animationDelay: "0.15s" }}>
      ...
      <div className="relative z-10">
        <h2 className="text-lg sm:text-2xl font-bold text-foreground mb-2 tracking-tight">
          Sem Limites no <span className="gradient-text">Plano Completo</span>
        </h2>
        <p className="text-xs sm:text-sm text-muted-foreground/60 mb-7 sm:mb-10 max-w-md leading-relaxed">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-5 sm:gap-6 mb-8 sm:mb-10">
          {benefits.map((b) => (
            <div key={b.label} className="flex items-start gap-3">
              ...
              <span className="text-xs sm:text-sm text-foreground/80 font-medium leading-tight">{b.label}</span>
            </div>
          ))}
        </div>
        ...
        <div className="flex justify-center">
          <Link
            to="/subscription"
            className="w-full sm:w-auto inline-flex items-center justify-center gap-2.5 rounded-2xl btn-gradient px-10 py-4 text-sm font-bold shadow-xl shadow-primary/15 active:scale-[0.97] hover:scale-[1.02] transition-transform duration-300 min-h-[50px] tracking-wide"
          >
