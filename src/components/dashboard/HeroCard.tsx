...
export function HeroCard({ firstName, isLoading, isDemo }: HeroCardProps) {
  ...
  return (
    <div className="rounded-2xl glass-card-strong p-6 sm:p-10 relative overflow-hidden glow-ring animate-fade-in">
      ...
      <div className="absolute -top-24 -right-24 w-52 sm:w-72 h-52 sm:h-72 rounded-full opacity-60 pointer-events-none"
        style={{ background: "radial-gradient(circle, hsl(255 80% 65% / 0.10), transparent 70%)" }} />
      <div className="absolute -bottom-16 -left-16 w-40 sm:w-56 h-40 sm:h-56 rounded-full opacity-50 pointer-events-none"
        style={{ background: "radial-gradient(circle, hsl(215 75% 58% / 0.08), transparent 70%)" }} />

      <div className="relative z-10 space-y-4">
        <h1 className="text-2xl sm:text-3xl md:text-[2.25rem] font-extrabold text-foreground leading-[1.15] tracking-tight">
          Olá, {firstName}! Bem-vindo ao{" "}
          <span className="gradient-text">UNITRIX</span>.
        </h1>

        {isTrial && days !== null ? (
          <p className="text-foreground/70 text-sm sm:text-base flex flex-wrap items-center gap-2 leading-relaxed">
            Seu
        ...
        <p className="text-muted-foreground/70 text-xs sm:text-sm max-w-lg leading-relaxed">
          {isTrial
